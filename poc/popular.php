...
<body>
	...
	<?php
		if(isset($_GET['idChaine'],$_GET['lang'],$_GET['newArticles'])){
			...
			$countquery ="select count(*) as number FROM Document WHERE id IN (SELECT idDoc FROM Doc_Chaine where idChaine ='$id_chaine') And  idLangue IN (select id from Langue where langue='en' or langue='$lang')";
        ...
        foreach($res as $l){
			...
			$docs_chaine = $pdo->prepare("SELECT * FROM Document WHERE id IN (SELECT idDoc FROM Doc_Chaine where idChaine =:idChaine) And  idLangue IN (select id from Langue where langue='en' or langue=:lang) ORDER BY idLangue desc, likes + 0 desc  LIMIT $limit, 6");
			...
			var articlesCount = 6;
			 ...
			 $('.more').click(function(){
				...
				if(articlesCount>count-7){$('.more').remove();}
 				...
 				articlesCount += 6;
 				$("."+divnum).load(
    "fetch_popular.php?idChaine="+id_chaine+"&lang="+langvar+"&newArticles="+articlesCount
);divnum++;}
     else {
