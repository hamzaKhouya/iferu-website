...
<div class="btn-plus">
	...
	<div class="articles">
	<?php
		if(isset($_GET['idChaine'],$_GET['lang'])){
			$id_chaine = $_GET['idChaine'] ? $_GET['idChaine'] : '';
			$lang = $_GET['lang'] ? $_GET['lang'] : '';
			$countquery ="select count(*) as number FROM Document WHERE id IN (SELECT idDoc FROM Doc_Chaine where idChaine ='$id_chaine') And idLangue IN (select id from Langue where langue='en' or langue='$lang') order by idLangue desc" ;
		...
		$nomlang='nom_'.$lang;
			...
			And idLangue IN (select id from Langue where langue='en' or langue=:lang) order by idLangue desc,titre desc Limit 6");
			...
			echo "<h2 class='title'>Chaine ".$nomChaine['title']."</span></h2>";
		    ...
		    echo "<input type='hidden' value='$maxcount' id='count'></input>";
		}else{
 		...
 		$(document).ready(function(){
			var articlesCount = 6;
			 ...
			 $('.more').click(function(){
				...
				if(articlesCount>count-7){$('.more').remove();}

				else if(divnum===2){
				
 				articlesCount += 6;
				 $("."+divnum).load("fetch_article_chaine.php?idChaine="+id_chaine+"&lang="+langvar+"&newArticles="+articlesCount);divnum++;}
     else {
