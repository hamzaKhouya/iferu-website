...
class SMTP {
    ...
    var $NomDuDomaine = 'maxwellconseil.com';

    // De Qui
    var $From = 'root@localhost';// Adresse de l' exp�diteur
    var $FromName = 'Root';// Nom de l' exp�diteur
    var $ReplyTo = '';// Adresse de retour
    var $org = 'Localhost'; // Organisation

    // A Qui
    var $To = '';
    // Utilisation : $Bcc = 'mail1,mail2,....';
    var $Bcc = '';// Blind Carbon Copy, c'est � dire que les adresses qui sont contenue ici seront invisibles pour tout le monde
    var $Cc = '';

    // Priorit�
    var $Priority = 3;// Priorit� accord�e au mail (valeur allant de 1 pour Urgent � 3 pour normal et 6 pour bas)

    // Encodage
    var $ContentType = 'txt';//Contenu du mail (texte, html...) (txt , html, txt/html)
    ...
    var $Confimation_reception = '';// Entrez l' adresse o� sera renvoy� la confirmation
    ...
    //**************************************************************************
    // Param�tre de connection SMTP
    //**************************************************************************
    var $Authentification_smtp = false;
    
    var $serveur = '';// Serveur SMTP
    var $port = 587;// Port SMTP
    var $login_smtp = '';// Login pour le serveur SMTP
    var $mdp_smtp = '';// Mot de passe pour le serveur SMTP
    var $time_out = 10;// Dur�e de la connection avec le serveur SMTP
    var $tls = false;// Activation de la connection s�curis�e (anciennement ssl)
...
//------------------------------------------------------------------------------
    ...
    //**************************************************************************
    // Fonction de d�claration de connection SMTP
    //**************************************************************************
    function SMTP($serveur='', $user='', $pass='', $port=587, $NomDuDomaine='', $debug=false){
        if($serveur){
    ...
    function Connect_SMTP(){
        ...
        $this->smtp_connection = fsockopen($this->serveur, // Serveur
                                     ...
                                     $num_erreur,    	   // Num�ros de l' erreur
                                     $msg_erreur,    	   // Message d' erreur
                                     $this->time_out);     // Dur�e de la connection en secs
        if(!$this->smtp_connection){// V�rification de la connection
            $this->erreur = 'Impossible de se connecter au serveur SMTP !!!<br />'."\r\n"
            .'Num�ro de l&#39; erreur: '.$num_erreur.'<br />'."\r\n"
            .'Message renvoy�: '.$msg_erreur.'<br />'."\r\n";
            return false;
        }
        ...
        // On r�gle le timeout du serveur SMTP car parfois, le serveur SMTP peut �tre un peut lent � r�pondre
        // Windows ne comprend pas la fonction socket_set_timeout donc on v�rifi que l' on travail sous Linux
        ...
        if($this->NomDuDomaine === ''){// On v�rifit si le nom de domaine � �t� renseign�
        ...
        if(!$this->Commande('EHLO '.$this->NomDuDomaine, 250)){// Commande EHLO
            // Deusi�me commande EHLO -> HELO
            if(!$this->Commande('HELO '.$this->NomDuDomaine, 250, 'Le serveur refuse l&#39; authentification (EHLO et HELO) !!!')){// Commande HELO
                return false;
            }
        }

        if($this->tls && !$this->Commande('STARTTLS', 220, 'Le serveur refuse la connection s�curis�e ( STARTTLS ) !!!')){// Commande STARTTLS
            return false;
        }
        
        if($this->Authentification_smtp){// On v�rifi si l' on a besoin de s' authentifier
            //******************************************************************
            // Authentification
            //******************************************************************
    ...
    //**************************************************************************
    // Foncton d' ajout de pi�ce jointe
    //**************************************************************************
    ...
    function headers(){
        ...
        // Adresse de l'exp�diteur (format : Nom <adresse_mail>)
        if(!empty($this->From)){
            $header .= 'X-Sender: '.$this->From."\n";// Adresse r�elle de l'exp�diteur
        }
		// La version mime
        if(!empty($this->MIME)){
        ...
        .'  boundary="'.$Boundary1.'"'."\n"
        // Logiciel utilis� pour l' envoi des mails
		...
		// Priorit� accord�e au mail (valeur allant de 1 pour Urgent � 3 pour normal et 6 pour bas)		
        ...
        // Bcc
        if(empty($this->Bcc)){// Blind Carbon Copy, c' est � dire que les adresses qui sont contenue ici seront invisibles pour tout le monde
            $No_body++;// Personne
        }
        // Sujet
        if(!empty($this->Sujet)){
        ...
        if(!empty($this->Confimation_reception)){// Adresse utilis�e pour la r�ponse au mail
            $header .= 'Disposition-Notification-To: <'.$this->Confimation_reception.'>'."\n";
        }
		// ReplyTo
		if(!empty($this->ReplyTo) && $this->ReplyTo !== $this->From && $this->ReplyTo !== 'root@localhost'){// Adresse utilis�e pour la r�ponse au mail
    ...
    function smtp_mail($to, $subject, $message, $header=''){
        // Pas de d�connection automatique
        $auto_disconnect = false;
        // On v�rifit si la connection existe
        if(empty($this->smtp_connection)){
            if(!$this->Connect_SMTP()){// Connection
                $this->erreur .= 'Impossible d&#39; envoyer le mail !!!<br />'."\r\n";
                return false;
            }
            $auto_disconnect = true;// D�connection automatique activ�e
        }
        
        // On v�rifit Que c' est le premier tour sinon on �fface les anciens param�tres
        if($this->Tour){
        ...
        //**********************************************************************
        // Variables temporairement modifi�es
        if(!empty($to)){
            $this->To = $to;
        }
        if(!empty($subject)){
        ...
        if(!empty($this->From) && !$this->Tour){
            if(!$this->Commande('MAIL FROM:<'.$this->From.'>', 250, 'Envoie du mail impossible car le serveur n&#39; acc�pte pas la commande MAIL FROM !!!')){
                return false;
            }
            $this->Tour = 1;
        }
        ...
        foreach($A as $cle => $tmp_to){
            if(substr_count($tmp_to, ',')){
                ...
                foreach($tmp_to as $cle => $tmp_A){
                    if(!$this->Commande('RCPT TO:<'.$tmp_A.'>', array(250,251), 'Envoie du mail impossible car le serveur n&#39; acc�pte pas la commande RCPT TO !!!')){
                        return false;
                    }
                }
            }else{
                if(!$this->Commande('RCPT TO:<'.$tmp_to.'>', array(250,251), 'Envoie du mail impossible car le serveur n&#39; acc�pte pas la commande RCPT TO !!!')){
                    return false;
                }
            }
        }
        ...
        //**********************************************************************
        // On cr�er les ent�tes ( headers ) si c' est pas fait
        if(empty($header)){
        ...
        //**********************************************************************
        // On indique que l' on va envoyer des donn�es
        if(!$this->Commande('DATA', 354, 'Envoie du mail impossible car le serveur n&#39; acc�pte pas la commande DATA!!!')){
            return false;
        }


        //**********************************************************************
        // Envoie de l' ent�te et du message
        ...
        //**********************************************************************
        // D�connection automatique
        //**********************************************************************
        if($auto_disconnect){// Auto d�connection ?
            $this->Deconnection_SMTP();// D�connection
        }
    ...
    //**************************************************************************
    // Lecture des donn�es renvoy�es par le serveur SMTP
    //**************************************************************************
    function get_smtp_data(){
        $data = '';
        while($donnees = fgets($this->smtp_connection, 515)){// On parcour les donn�es renvoy�es
            $data .= $donnees;

            if(substr($donnees,3,1) == ' ' && !empty($data)){break;}// On v�rifi si on a toutes les donn�es
        }
        // Renvoie des donn�es : array(Code, message complet)
    ...
    function Commande($commande, $bad_error, $msg_error='', $debug=1){
        if(!empty($this->smtp_connection)){
            ...
            $this->erreur = 'Impossible d&#39; �xecuter la commande <span style="font-weight:bolder;">'.$commande.'</span> car il n&#39; y a pas de connection !!!<br />'."\r\n";
            if(!$debug){
    ...
    //**************************************************************************
    // Fonction de d�connection SMTP
    //**************************************************************************
    function Deconnection_SMTP(){
        if(!empty($this->smtp_connection)){
            if(!$this->Commande('QUIT', 221, 'Impossible de se d�connecter !!!')){
                return false;
            }

            @sleep(5);// On laisse 5 seconde au serveur pour terminer toutes les instructions
            if(!fclose($this->smtp_connection)){
                $this->erreur = 'Impossible de se d�connecter !!!<br />'."\r\n";
                return false;
            }
            $this->smtp_connection = 0;
            return true;            
        }
        $this->erreur = 'Impossible de se d�connecter car il n&#39; y a pas de connection !!!<br />'."\r\n";
        return false;
    }
}
?>
